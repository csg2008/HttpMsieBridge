...
const int DISPID_BASE = 2000;
const int DISPID_ENTERGAME = 10001;
...
HRESULT STDMETHODCALLTYPE ExternalDispatch::GetIDsOfNames(
    ...
    int functionCount = (int) javaScriptFunctions.size();
...
HRESULT STDMETHODCALLTYPE ExternalDispatch::Invoke(
    ...
    /*
    if (dispId<DISPID_BASE || dispId>DISPID_BASE + javaScriptFunctions.size()) {
        return E_NOTIMPL;
    }	
    ...
    return javaScriptFunctions[dispId-DISPID_BASE].cppFunctionPtr(dispId, pDispParams, pVarResult);
    */
...
void ExternalDispatch::AddJavaScriptFunction(wchar_t * functionName, ApiToJavaScript cppFunctionPtr) {
	JavaScriptFunction jsFunc;
	jsFunc.jsFunctionName = functionName;
	jsFunc.jsFunctionID = DISPID_BASE + (int) javaScriptFunctions.size();
