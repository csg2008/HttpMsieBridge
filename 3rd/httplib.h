...
//  rev: 2019-09-07 531708816a2381bf60d41ddf650f486a3bf00864
...
struct Request {
  ...
  bool has_form(const char *key) const;
  std::string get_form_value(const char *key, size_t id = 0) const;
  size_t get_form_value_count(const char *key) const;
...
inline bool Request::has_form(const char *key) const {
  return form.find(key) != form.end();
}

inline std::string Request::get_form_value(const char *key, size_t id) const {
...
inline size_t Request::get_form_value_count(const char *key) const {
  auto r = form.equal_range(key);
  return std::distance(r.first, r.second);
}
...
Server::process_request(Stream &strm, bool last_connection,
  ...
  if (req.method == "POST" || req.method == "PUT" || req.method == "PATCH") {
    ...
    if (!content_type.find("application/x-www-form-urlencoded")) {
      detail::parse_query_text(req.body, req.form);
